...
### Post-dispatch verification (v0.21.0+)

After every `task(subagent_type=...)` call returns, BEFORE declaring the phase done:

1. **Verify the artifact exists** at the expected path (use `glob` or `read` to check). The expected path follows the convention `share/notes/<phase>_<topic>_<task-id>.md` or per-phase equivalent.
2. **If the dispatch aborted** (no return message, error, or artifact missing): re-dispatch once with a slightly tighter prompt, OR surface to user with `NEEDS_USER_INPUT: true`.
3. **Append a `phase_done` entry** to `share/notes/00_trace_<task-id>.jsonl` with the artifact path + size + timestamp. This is the audit trail; the user can replay what actually shipped.
4. **Only then** advance to the next phase.

Why this exists: an aborted `task()` call leaves master with no signal that work shipped vs didn't. Without verification, master proceeds on assumption. The trace JSONL becomes the source of truth.

Also: after every edit/write batch (not just dispatches), run `grep -rn $'\u2014' <files-touched>` over the touched files to find any em-dash character (U+2014). Replace per the em-dash rule in `CLAUDE.md` § Voice.
